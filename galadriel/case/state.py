...
class ScenarioState(rx.State):
    scenarios: List['CaseModel'] = []
    scenario: Optional['CaseModel'] = None
    ...
    def get_scenario_detail(self):                
        with rx.session() as session:
            if (self.scenario_id == ""):
                self.scenario = None
                return            
            result = session.exec(CaseModel.select().where(CaseModel.id == self.scenario_id)).one_or_none()
    ...
    def save_scenario_edits(self, scenario_id:int, updated_data:dict):
        with rx.session() as session:        
            scenario = session.exec(CaseModel.select().where(CaseModel.id == scenario_id)).one_or_none()
