    ...
    return rx.fragment(
        rx.form(
            rx.table.root(
                ...
                rx.table.body(rx.foreach(state.CycleState.linkable_cases_for_search, __show_test_cases_in_search)),
                variant="surface",
                size="3",
                width="100%",
            ...
            rx.cond(
                state.CycleState.empty_cases_for_search.length() > 0,
                rx.vstack(
                    rx.text("Cases without steps (not available to add)", size="2", color="var(--gray-8)", padding_top="1em"),
                    rx.table.root(
                        rx.table.body(rx.foreach(state.CycleState.empty_cases_for_search, __show_empty_case_in_search)),
                        variant="surface",
                        size="3",
                        width="100%",
                    ),
