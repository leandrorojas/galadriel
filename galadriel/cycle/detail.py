...
def __search_table_header():
    ...
    return rx.table.header(
        rx.table.row(
            ...
            Table.sortable_header("name", "fingerprint", "name", state.CycleState.search_sort_by, state.CycleState.search_sort_asc, state.CycleState.toggle_search_sort),
...
def __show_test_cases_in_search(test_case:CaseModel):
    ...
    return rx.table.row(
            ...
            rx.table.cell(rx.el.input(name="case_id", type="hidden", value=rx.cond(test_case.id, test_case.id, "")), hidden=True),
    )
    ...
    return rx.fragment(
        rx.form(
            rx.table.root(
                ...
                rx.table.body(rx.foreach(state.CycleState.sorted_cases_for_search, __show_test_cases_in_search)),
                variant="surface",
                size="3",
                width="100%",
...
def __show_scenarios_in_search(scenario:ScenarioModel):
    ...
    return rx.table.row(
            ...
            rx.table.cell(rx.el.input(name="scenario_id", type="hidden", value=rx.cond(scenario.id, scenario.id, "")), hidden=True),
    )
...
def __search_scenarios_table() -> rx.Component:
    return rx.fragment(
        rx.form(
            rx.table.root(
                __search_table_header(),
                rx.table.body(rx.foreach(state.CycleState.sorted_scenarios_for_search, __show_scenarios_in_search)),
                variant="surface",
                size="3",
                width="100%",
...
def __show_suites_in_search(suite:SuiteModel):
    ...
    return rx.table.row(
            ...
            rx.table.cell(rx.el.input(name="suite_id", type="hidden", value=rx.cond(suite.id, suite.id, "")), hidden=True),
    )
...
def __search_suites_table() -> rx.Component:
    return rx.fragment(
        rx.form(
            rx.table.root(
                __search_table_header(),
                rx.table.body(rx.foreach(state.CycleState.sorted_suites_for_search, __show_suites_in_search)),
                variant="surface",
                size="3",
                width="100%",
