...
def __show_test_cases(test_cases:model.ScenarioCaseModel):
    ...
    return rx.table.row(
        ...
        rx.table.cell(
            rx.flex(
                rx.button(rx.icon("arrow-big-up"), disabled=(DISABLE_EDIT_MODE | (test_cases.order == 1)), on_click=lambda: state.ScenarioState.move_case_up(getattr(test_cases, consts.FIELD_ID))),  # NOSONAR - Reflex event handler; self is implicit
                rx.button(rx.icon("arrow-big-down"), disabled=(DISABLE_EDIT_MODE | (test_cases.order == state.ScenarioState.test_case_count)), on_click=lambda: state.ScenarioState.move_case_down(getattr(test_cases, consts.FIELD_ID))),  # NOSONAR
    ...
    scenario_detail_content = rx.vstack(
        ...
        rx.vstack(
            ...
            rx.cond(
                ...
                rx.box(
                        rx.box(rx.el.input(type="hidden", name="scenario_id", value=rx.cond(scenario.id, scenario.id, "")), display="none",),
                        rx.vstack(
