...
def __show_test_cases_in_search(test_case:CaseModel):
    ...
    return rx.table.row(
            ...
            rx.table.cell(rx.form(rx.input(type="number", name="case_id", value=rx.cond(test_case.id, test_case.id, ""))), hidden=True),
    )
...
def __show_scenarios_in_search(scenario:ScenarioModel):
    ...
    return rx.table.row(
            ...
            rx.table.cell(rx.form(rx.input(type="number", name="scenario_id", value=rx.cond(scenario.id, scenario.id, ""))), hidden=True),
    )
...
def __show_child(suite_child:model.SuiteChildModel):
    ...
    return rx.table.row(
        ...
        rx.table.cell(
            rx.flex(
                rx.button(rx.icon("arrow-big-up"), disabled=(DISABLE_EDIT_MODE | (suite_child.order == 1)), on_click=lambda: state.SuiteState.move_child_up(getattr(suite_child, consts.FIELD_ID))),  # NOSONAR - Reflex event handler; self is implicit
                rx.button(rx.icon("arrow-big-down"), disabled=(DISABLE_EDIT_MODE | (suite_child.order == state.SuiteState.child_count)), on_click=lambda: state.SuiteState.move_child_down(getattr(suite_child, consts.FIELD_ID))),  # NOSONAR
